...
class DataDrawable extends BasicDrawable {
  ...
  function getMoon() {
    ...
    if (
      additionalValue[0].add(new Time.Duration(43200)).lessThan(today) ||
      additionalValue[1] == null
    ) {
      additionalValue[0] = today;
      additionalValue[1] = Moon.moonPhase(today);
      image = Moon.drawMoon(
        additionalValue[1][:IP1],
        height,
        foregroundColor(),
        backgroundColor()
      );
  ...
  function getSecondTime() {
    ...
    return Global.momentToString(
      Time.now().add(new Time.Duration(additionalValue))
    );
  }
