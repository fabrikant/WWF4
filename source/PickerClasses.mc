...
class StringPicker extends WatchUi.Picker {
    ...
    function initialize(parentWeak, mCharacterSet) {
    	
    	self.parentWeak = parentWeak;
    	self.mCharacterSet = mCharacterSet;
        mFactory = new CharacterFactory(mCharacterSet, {:addOk=>true});
        mTitleText = "";
		if (parentWeak.stillAlive()){
    		var obj = parentWeak.get();
     		mTitleText = obj.getSubLabel();
    	}
        var defaults = null;
        defaults = [mFactory.getIndex(mTitleText.substring(mTitleText.length()-1, mTitleText.length()))];
...
class StringPickerDelegate extends WatchUi.PickerDelegate {
    ...
    function onAccept(values) {
        if(!mPicker.isDone(values[0])) {
        	...
        	if (mPicker.parentWeak.stillAlive()){
        		var obj = mPicker.parentWeak.get();
        		obj.onSetText(value);
        	}
            WatchUi.popView(WatchUi.SLIDE_IMMEDIATE);
        }
