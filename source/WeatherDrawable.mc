...
class WidgetDrawable extends TopBottomDrawable {
  ...
  public function draw(dc as Graphics.Dc) {
    ...
    if (dataType == WEATHER) {
      if (lastWeatherUpdate == null) {
        ...
        var responseCode = Application.Storage.getValue(
        ...
        if (geoLatLong[0] == null || geoLatLong[1] == null) {
          ...
          drawNoData(
            dc,
            Lang.format("$1$: $2$", [
              Application.loadResource(Rez.Strings.Error),
              responseCode,
            ])
          );
        } else {
  ...
  function readWeather() {
    ...
    var res = findResByCode(
      Application.Storage.getValue(STORAGE_KEY_WEATHER_ID),
      Application.Storage.getValue(STORAGE_KEY_ICON)
    );
    if (res == null) {
      res = Rez.Drawables.NA;
    }
    var image = createImage(res);
    var temp = Global.temperatureToString(
      Application.Storage.getValue(STORAGE_KEY_TEMP)
    );
    if (!temp.equals("")) {
      temp = temp + "°";
    }
    var direction = Application.Storage.getValue(STORAGE_KEY_WIND_DEG);
    var speed = Global.speedToString(
      Application.Storage.getValue(STORAGE_KEY_WIND_SPEED)
    );
    ...
    dc.fillRoundedRectangle(
      0,
      0,
      width + Sizes.radiusCorner() + 1,
      height + Sizes.radiusCorner() + 1,
      Sizes.radiusCorner()
    );
  ...
  private function windDirection(size, angle, leftTop, fieldSize) {
    ...
    var centerPoint = [
      leftTop[0] + fieldSize[0] / 2,
      leftTop[1] + fieldSize[1] / 2,
    ];
    var coords = [
      [(-size * 3) / 8, -size / 2],
      [0, size / 2],
